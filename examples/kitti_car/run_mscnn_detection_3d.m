...
%rootDir='mscnn-8s-768-mccnn-80-avg';
rootDir='crl-dispMap/trainval_mscnn-7s-576-2x-crl-eqHist-avg';
%rootDir='mscnn-7s-576-2x-VGG_scratch';
ori=' ';
%display(rootDir)
...
root_dir = strcat(rootDir,'/'); %'mscnn-8s-768-trainval-displets_addWeighted_imageProc';

%display (root_dir);

%binary_file = [root_dir 'mscnn_kitti_train_10k_2nd_iter_30000.caffemodel'];
...
% PEDRO
disp('Initializing the net (caffe.Net)');
%pause;
...
for k = 1:nImg
  ...
  % Pedro <begin>
  %imwrite(final_matrix(:,:,1),'matrix1.png');
  %imwrite(final_matrix(:,:,2),'matrix2.png');
  %imwrite(final_matrix(:,:,3),'matrix3.png');
  %imwrite(final_matrix(:,:,4),'matrix4.png');
  %pause;
  % Pedro <end>
 
  if (show)
    %imshow(test_image,'parent',h.axes); axis(h.axes,'image','off'); hold(h.axes,'on');
    imshow(final_matrix,'parent',h.axes); axis(h.axes,'image','off'); hold(h.axes,'on');
    pause;
  end
