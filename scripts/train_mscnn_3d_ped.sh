...
if [ ! -d "$mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_70" ]; then
	
	ln -s $kitti_Processed/mc-cnn/image_proc_0_70 $kitti_Root/leftCamera/training/image_2
	cp -R $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-trainval-ori/ $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_70/

	# Train MSCNN network
	cd $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_70
	sh train_mscnn.sh
...
if [ ! -d "$mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_80" ]; then

        echo ln -s $kitti_Processed/mc-cnn/image_proc_0_80 $kitti_Root/leftCamera/training/image_2
        echo cp -R $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-trainval-ori/ $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_80/

        # Train MSCNN network
        echo cd $mscnn_Root/examples/kitti_ped_cyc/mscnn-8s-768-image_proc_0_80
        sh train_mscnn.sh
