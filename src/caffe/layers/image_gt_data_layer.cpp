...
void ImageGtDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do {
    ...
    // PEDRO
    // LOG(INFO) << "---> Channels from image: image_size[0] = " << channels;
    // END PEDRO
  ...
  // PEDRO
  LOG(INFO) << " --> has_mean_values_ = " << has_mean_values_;
  // END PEDRO
  ...
  if (has_mean_values_) {
    ...
    CHECK(mean_values_.size() == 1 || mean_values_.size() == channels) <<
     "Specify either 1 mean_value or as many as channels: " << channels << " - [mean_values_.size() = " << mean_values_.size() << "]";
...
void ImageGtDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int dummy = 0; dummy < batch_size; ++dummy) {
	...
	// PEDRO
        ...
        cv_img = cv::imread(image.first, CV_LOAD_IMAGE_UNCHANGED);	
	//LOG(INFO) << "Reading image: " << image.first;
	// END PEDRO
        if (!cv_img.data) {
      ...
      // PEDRO
      LOG(INFO) << "Starting debug mode. Dumping data to disk at dump/";
      // END PEDRO
      string file_id;
      std::stringstream ss;
      ss << list_id_;
      ...
      std::ofstream top_data_file((root_dir + file_id + string("_data.txt")).c_str(),
          std::ofstream::out);
      // PEDRO
      LOG(INFO) << "Channels: " << channels << "\tWidth: " << template_width << "\tHeight: " << template_height;
      // END PEDRO
      for (int c = 0; c < channels; ++c) {
